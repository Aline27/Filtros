...
def filter_median (tam_filter,matriz_aux,img_size):

	size_matriz=matriz_aux.shape
	img_filtered2=np.zeros((img_size[0],img_size[1],3), dtype=np.uint8)

	for k_mat in range (0,3):
		print k_mat
		for i_mat in range(0,size_matriz[0]-tam_filter+1):
			for j_mat in range(0,size_matriz[1]-tam_filter+1):

				aux= matriz_aux[i_mat:i_mat+tam_filter,j_mat:j_mat+tam_filter,k_mat]
				aux = np.median(aux)

				img_filtered2[i_mat,j_mat,k_mat]=aux
	return img_filtered2

#---------------------------------------------------------------------------------------------------------

arq = open('filtro7.txt', 'r')
text = arq.read()
print(text)
arq.close()
...
img_filtered_median=filter_median(tam,matriz_result,img_size)
plt.imshow(img_filtered_median)
plt.show()
